...
<%
	...
	*/
	...
	//FileItem은 클라이언트의 전송 정보 하나 하나를 의미 즉, html에서의 input 텍스트박스, file컴포넌트 들..
	...
	for(FileItem item:items){
		...
		//반복문으로 처리되다 보니, 파일만 따로 처리를 하려면, 구분로직이 필요
		...
		if(!item.isFormField()){//텍스트박스가 아닌 것만 업로드 처리
			//업로드처리!메모리상의 이미지 정보를 실제 물리적 파일로 저장하자
			//새롭게 생성할 파일명
			...
			out.print("생성파일명: "+filename+"<br>");
			out.print("저장된 위치: "+saveDir+"<br>");
			out.print("파일크기: "+item.getSize()+"bytes <br>");
		}
	}
