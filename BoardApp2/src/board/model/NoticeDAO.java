...
public class NoticeDAO {
	...
	public ArrayList selectAll() {
		...
		try {
			...
			//rs는 레코드가 여러개 이므로 VO또한 여러개가 필요하고 이 VO를 한꺼번에 모아서
			//반환해야 하므로 집합형 자료형이 필요하다 객체를 모아놓은 집합을 지원하는 프레임웍은 CollectionFramework이므로
			//이 중 하나의 api를 이용
			//Notice notice=null을 하면 대체를 해버리기때문에 안에넣고 새로 생성해야함
			while(rs.next()) {
				Notice notice=new Notice();//텅 빈empty상태의 vo생성
	...
	public Notice select(int notice_id) {
		...
		con=dbManager.getConnection();//접속객체
		try {
			...
			//rs는 레코드 한 건을 담는 객체이므로, 레코드 1건을 담아전달용으로 사용되는 VO를 이용하자
			if(rs.next()) {//레코드가 존재하는 것임! 따라서 이때 VO를 올리자
				notice=new Notice();//텅 빈empty상태의 vo생성
				//notice에 rs의 정보를 모두 옮겨심자
				notice.setNotice_id(rs.getInt("notice_id"));
				notice.setAuthor(rs.getString("author"));
				notice.setTitle(rs.getString("title"));
